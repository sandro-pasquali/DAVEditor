...
var MochaUI = new Hash({
	...
	*/
	...
	setUnderlaySize: function(g){
	  /**
	   * sandro 05.10.09
	   * Seems that IE is firing a resize event, and if that fires prior to the
	   * document being ready, an error occurs.  Checking for existence of 
	   * element first, as a result.
	   */
	  if($('windowUnderlay'))
	    {
...
MochaUI.Modal = new Class({
	...
	modalInitialize: function(){
		...
		modalOverlay.addEvent('click', function(e){
		  /**
		   * Do not allow click-closing for confirm 
		   */
		  if(MochaUI.currentModal.id != '__confirm__')
		    {
...
MochaUI.Panel = new Class({
    ...
    /**
     * If not minimizable, don't show toggle.
     */
