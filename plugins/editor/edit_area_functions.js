			  ...
			  var sP = function()
			    {
			      if(p.editAreaLoader)
			        {
			          p.editAreaLoader.execCommand('"+ t.id +"', 'update_size();');editArea.focus();
			        }  
			    }
	...
	EditArea.prototype.open_file= function(settings){
		...
		if(settings['id']!="undefined")
		{
			...
			if(!_$(this.files[id]['html_id']) && id!="")
			{
			  /**
			   * Sandro 01.10.09
			   * Changed tab display, removing full path, showing only
			   * resource name.
			   */
			  var tds = this.files[id]['title'].split('/');
			  var tabDisplayName = decodeURIComponent(tds[tds.length-1]);
				...
				elem.innerHTML= "<a onclick=\"javascript:editArea.execCommand('switch_to_file', editArea.filesIdAssoc['"+ html_id +"']);\" selec=\"none\"><b><span><strong class=\"edited\">*</strong>"+ tabDisplayName + close +"</span></b></a>";
